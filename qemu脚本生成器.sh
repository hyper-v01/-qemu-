...
{  #Errors
	if (( $1 == 0 )); then
		echo "Error($1) : 你似乎缺少了qemu-tuils,qemu-system-x86-64和qemu-system-arm的某个"
		exit
	else 
		if (( $1 == 1)); then
...
function check_qemu3	#找qemu
{   #3
	whence -qp qemu-system-x86_64
	whence_process ${?}
	if (( $? == 0)); then
                Error 0
        fi
...
function check_qemu2	#找qemu
{    #2    
	whence -qp qemu-system-arm
        whence_process ${?}
	if (( $? == 1)); then
		check_qemu3
	else
...
function whence_process	#找qemu的结果的处理
{  #processing the whence
	if [ $1 == 0 ]; then
		return 1
	else
		if [ $1 == 1 ]; then
